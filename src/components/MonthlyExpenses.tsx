...
type CellField = 'description' | 'clientName' | 'amount' | 'date' | 'category';
...
export default function MonthlyExpenses({ tasks }: MonthlyExpensesProps) {
  ...
  // Combined category sidebar + selected category's items (mirrors the Team Tasks sections
  // layout): one sidebar lists both expense and income categories, the selection carries
  // which type so "Other" can exist on both sides without colliding.
  const [selectedEntry, setSelectedEntry] = useState<{ type: 'expense' | 'income'; name: string } | null>(null);
  ...
  const monthCounts = useMemo(() => {
    ...
    yearExpenses.forEach(item => { counts[item.date.getMonth()] += 1; });
    yearIncome.forEach(item => { counts[item.date.getMonth()] += 1; });
    return counts;
  ...
  // Income categories aren't a managed list like expense categories (no IncomeCategory
  // model) - just the fixed set, plus any legacy category still in use.
  const incomeCategoryGroups = useMemo(() => {
    const names = Array.from(new Set([...incomeCategories, ...monthIncome.map(i => i.category)]));
    return names.map(name => {
      const items = monthIncome.filter(i => i.category === name);
      return { name, items, subtotal: items.reduce((sum, i) => sum + i.amount, 0) };
    });
  }, [monthIncome]);

  useEffect(() => {
    const all: { type: 'expense' | 'income'; name: string }[] = [
      ...expenseCategoryGroups.map(g => ({ type: 'expense' as const, name: g.name })),
      ...incomeCategoryGroups.map(g => ({ type: 'income' as const, name: g.name })),
    ];
    if (!all.length) { if (selectedEntry !== null) setSelectedEntry(null); return; }
    if (!selectedEntry || !all.some(e => e.type === selectedEntry.type && e.name === selectedEntry.name)) {
      setSelectedEntry(all[0]);
    }
  }, [expenseCategoryGroups, incomeCategoryGroups, selectedEntry]);

  const selectedExpenseGroup = selectedEntry?.type === 'expense' ? expenseCategoryGroups.find(g => g.name === selectedEntry.name) ?? null : null;
  const selectedIncomeGroup = selectedEntry?.type === 'income' ? incomeCategoryGroups.find(g => g.name === selectedEntry.name) ?? null : null;
  ...
  const openAddExpenseForm = () => {
    resetExpenseForm();
    if (selectedEntry?.type === 'expense') setExpCategory(selectedEntry.name);
  ...
  const submitAddCategory = async (event: React.FormEvent) => {
    ...
    if (created) {
      ...
      setSelectedEntry({ type: 'expense', name });
    }
  };
  ...
  const commitRenameCategory = async (name: string) => {
    ...
    if (ok) setSelectedEntry({ type: 'expense', name: value });
  };
  ...
  const commitExpenseCell = async (expense: Expense, field: CellField, raw: string) => {
    setEditCell(null);
  ...
  const openAddIncomeForm = () => {
    resetIncomeForm();
    if (selectedEntry?.type === 'income') setIncCategory(selectedEntry.name);
    const month = selectedMonth === 'all' ? now.getMonth() : selectedMonth;
  ...
  const patchIncome = async (id: string, payload: Record<string, unknown>, apply: (item: IncomeItem) => IncomeItem) => {
    setRowPendingId(id);
    try {
      ...
      const result = await response.json();
      if (!result.success) throw new Error(result.error || 'Failed to update payment');
      setIncome(prev => prev.map(i => (i.id === id ? apply(i) : i)));
    } catch (err) {
  ...
  const commitIncomeCell = async (item: IncomeItem, field: CellField, raw: string) => {
    setEditCell(null);
    if (field === 'description') {
      const value = raw.trim();
      if (!value || value === item.description) return;
      await patchIncome(item.id, { description: value }, i => ({ ...i, description: value }));
      return;
    }
    if (field === 'clientName') {
      const value = raw.trim();
      if (value === item.clientName) return;
      await patchIncome(item.id, { clientName: value }, i => ({ ...i, clientName: value }));
      return;
    }
    if (field === 'amount') {
      const value = parseFloat(raw);
      if (isNaN(value) || value < 0 || value === item.amount) return;
      await patchIncome(item.id, { amount: value }, i => ({ ...i, amount: value }));
      return;
    }
    if (field === 'category') {
      if (!raw || raw === item.category) return;
      await patchIncome(item.id, { category: raw }, i => ({ ...i, category: raw }));
      return;
    }
    // date
    if (!raw || raw === item.date.toISOString().split('T')[0]) return;
    await patchIncome(item.id, { date: raw }, i => ({ ...i, date: new Date(raw) }));
    const targetDate = new Date(raw);
    if (targetDate.getFullYear() !== selectedYear) {
      setSelectedYear(targetDate.getFullYear());
      setSelectedMonth(targetDate.getMonth());
  ...
  const submitQuickIncome = async (categoryName: string) => {
    const description = quickDescription.trim();
    const amount = parseFloat(quickAmount);
    if (!description || isNaN(amount) || amount < 0 || quickBusy) return;
    setQuickBusy(true);
    try {
      ...
      const response = await fetch('/api/income', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ description, amount, category: categoryName, date, clientName: '' }),
      });
      const result = await response.json();
      if (!result.success) throw new Error(result.error || 'Failed to add payment');
      const created = result.data as { _id: string; description: string; clientName?: string; amount: number; category: string; date: string; notes?: string; createdBy?: string };
      setIncome(prev => [{ id: created._id, description: created.description, clientName: created.clientName || '', amount: created.amount, category: created.category, date: new Date(created.date), notes: created.notes || '', createdBy: created.createdBy }, ...prev]);
      setQuickDescription('');
      setQuickAmount('');
    } catch (err) {
  ...
  const expenseRow = (expense: Expense) => {
    ...
    const isEditing = (field: CellField) => editCell?.id === expense.id && editCell.field === field;
    return (
  ...
  const incomeRow = (item: IncomeItem) => {
    const pending = rowPendingId === item.id;
    const isEditing = (field: CellField) => editCell?.id === item.id && editCell.field === field;
    return (
      <div key={item.id} className={`group flex flex-wrap items-center gap-2 border-t border-gray-100 px-4 py-2 hover:bg-gray-50 sm:flex-nowrap ${pending ? 'opacity-60' : ''}`}>
        <div className="min-w-0 flex-1 basis-full sm:basis-auto">
          {isEditing('description') ? (
            <input
              autoFocus
              ...
              maxLength={200}
              onKeyDown={e => { if (e.key === 'Enter') { e.preventDefault(); e.currentTarget.blur(); } else if (e.key === 'Escape') { e.currentTarget.value = item.description; e.currentTarget.blur(); } }}
              onBlur={e => commitIncomeCell(item, 'description', e.target.value)}
              className="w-full rounded border border-gray-300 px-2 py-1 text-sm text-gray-900 outline-none focus:border-emerald-500"
            />
          ) : (
            <button type="button" onClick={() => setEditCell({ id: item.id, field: 'description' })} className="block w-full truncate rounded px-1 py-0.5 text-left text-sm font-medium text-gray-900 hover:bg-gray-100">
              {item.description}
            </button>
          )}
        </div>
        ...
        <div className="w-32 shrink-0">
          {isEditing('clientName') ? (
            <input
              ...
              maxLength={200}
              onKeyDown={e => { if (e.key === 'Enter') { e.preventDefault(); e.currentTarget.blur(); } else if (e.key === 'Escape') { e.currentTarget.value = item.clientName; e.currentTarget.blur(); } }}
              onBlur={e => commitIncomeCell(item, 'clientName', e.target.value)}
              className="w-full rounded border border-gray-300 px-2 py-1 text-sm text-gray-900 outline-none focus:border-emerald-500"
            />
          ) : (
            <button type="button" onClick={() => setEditCell({ id: item.id, field: 'clientName' })} className="block w-full truncate rounded px-1 py-0.5 text-left text-sm text-gray-600 hover:bg-gray-100">
              {item.clientName || '—'}
            </button>
          )}
        </div>
        ...
        <div className="w-28 shrink-0">
          {isEditing('date') ? (
            <input
              type="date"
              autoFocus
              defaultValue={item.date.toISOString().split('T')[0]}
              onChange={e => { if (e.target.value) commitIncomeCell(item, 'date', e.target.value); }}
              onBlur={e => commitIncomeCell(item, 'date', e.target.value)}
              className="w-full rounded border border-gray-300 px-2 py-1 text-sm text-gray-900 outline-none focus:border-emerald-500"
            />
          ) : (
            <button type="button" onClick={() => setEditCell({ id: item.id, field: 'date' })} className="rounded px-1 py-0.5 text-sm text-gray-600 hover:bg-gray-100">
        ...
        <div className="w-32 shrink-0">
          {isEditing('category') ? (
            <select
              ...
              onChange={e => commitIncomeCell(item, 'category', e.target.value)}
              onBlur={() => setEditCell(null)}
              className="w-full rounded border border-gray-300 px-1.5 py-1 text-xs text-gray-900 outline-none focus:border-emerald-500"
            >
              {incomeCategories.map(c => <option key={c} value={c}>{c}</option>)}
            </select>
          ) : (
            <button type="button" onClick={() => setEditCell({ id: item.id, field: 'category' })}>
              <span className="inline-block rounded px-2 py-1 text-xs bg-emerald-50 text-emerald-700 hover:bg-emerald-100">{item.category}</span>
            </button>
          )}
        </div>
        ...
        <div className="w-24 shrink-0 text-right">
          {isEditing('amount') ? (
            <input
              ...
              onKeyDown={e => { if (e.key === 'Enter') { e.preventDefault(); e.currentTarget.blur(); } else if (e.key === 'Escape') { e.currentTarget.value = String(item.amount); e.currentTarget.blur(); } }}
              onBlur={e => commitIncomeCell(item, 'amount', e.target.value)}
              className="w-full rounded border border-gray-300 px-2 py-1 text-right text-sm text-gray-900 outline-none focus:border-emerald-500"
            />
          ) : (
            <button type="button" onClick={() => setEditCell({ id: item.id, field: 'amount' })} className="w-full rounded px-1 py-0.5 text-right text-sm font-semibold text-gray-900 hover:bg-gray-100">
        ...
        <div className="flex w-16 shrink-0 items-center justify-end gap-0.5 text-gray-400 opacity-0 transition-opacity focus-within:opacity-100 group-hover:opacity-100">
          <button type="button" onClick={() => openEditIncomeForm(item)} title="Edit details / notes" className="rounded p-1.5 hover:bg-gray-200 hover:text-gray-700"><PencilIcon /></button>
          <button type="button" onClick={() => handleDeleteIncome(item)} title="Delete" className="rounded p-1.5 hover:bg-red-100 hover:text-red-600"><TrashIcon /></button>
        </div>
      </div>
    );
  };

  const categoryQuickAdd = (type: 'expense' | 'income', categoryName: string) => {
    const submit = type === 'expense' ? submitQuickExpense : submitQuickIncome;
    const accent = type === 'expense' ? 'bg-blue-600 hover:bg-blue-700 focus:border-blue-500' : 'bg-emerald-600 hover:bg-emerald-700 focus:border-emerald-500';
    const label = type === 'expense' ? 'Add expense' : 'Add payment';
    return quickAddCategory === categoryName ? (
      <div className="flex flex-wrap items-center gap-2 border-t border-gray-100 px-4 py-2">
        <input
          ...
          maxLength={200}
          onKeyDown={e => { if (e.key === 'Enter') { e.preventDefault(); submit(categoryName); } else if (e.key === 'Escape') { setQuickAddCategory(null); } }}
          className={`min-w-0 flex-1 rounded border border-gray-300 px-2.5 py-1.5 text-sm text-gray-900 outline-none ${type === 'expense' ? 'focus:border-blue-500' : 'focus:border-emerald-500'}`}
        />
        <input
          value={quickAmount}
          ...
          min="0"
          onKeyDown={e => { if (e.key === 'Enter') { e.preventDefault(); submit(categoryName); } else if (e.key === 'Escape') { setQuickAddCategory(null); } }}
          className={`w-24 rounded border border-gray-300 px-2.5 py-1.5 text-sm text-gray-900 outline-none ${type === 'expense' ? 'focus:border-blue-500' : 'focus:border-emerald-500'}`}
        />
        <button type="button" onClick={() => submit(categoryName)} disabled={quickBusy || !quickDescription.trim() || !quickAmount} className={`rounded px-3 py-1.5 text-sm font-medium text-white disabled:opacity-50 ${accent}`}>{quickBusy ? 'Adding…' : 'Add'}</button>
  ...
  return (
    <div className="p-6">
      ...
      <div className="flex items-center gap-1 overflow-x-auto mb-6 pb-1">
        ...
        >
          ...
          <span className={`ml-1 px-1.5 py-0.5 rounded text-xs ${selectedMonth === 'all' ? 'bg-gray-700 text-gray-200' : 'bg-gray-200 text-gray-600'}`}>
            {yearExpenses.length + yearIncome.length}
          </span>
        </button>
      ...
      {/* Combined category sidebar + selected category's items, mirroring the Team Tasks sections layout */}
      <div>
        {loading ? (
          <div className="rounded-lg border border-gray-200 bg-white p-6 text-center text-gray-500">Loading...</div>
        ) : expenseCategoryGroups.length === 0 && incomeCategoryGroups.length === 0 ? (
          <div className="rounded-lg border border-gray-200 bg-white p-6 text-center text-gray-500">
            {selectedMonth === 'all' ? `No expenses or payments recorded for ${selectedYear} yet.` : 'No expenses or payments recorded for this month yet.'}
          </div>
        ) : (
            ...
            <aside className="flex shrink-0 flex-col rounded-lg border border-gray-200 bg-white lg:w-72">
              <nav className="flex flex-col gap-0.5 p-1.5">
                {expenseCategoryGroups.length > 0 && (
                  <>
                    <p className="px-2.5 pb-1 pt-2 text-[11px] font-semibold uppercase tracking-wide text-gray-400">Expenses</p>
                    {expenseCategoryGroups.map(group => {
                      const active = selectedEntry?.type === 'expense' && selectedEntry.name === group.name;
                      return (
                        <button
                          key={`expense-${group.name}`}
                          type="button"
                          onClick={() => setSelectedEntry({ type: 'expense', name: group.name })}
                          className={`flex items-center justify-between gap-2 rounded-lg px-2.5 py-2 text-left text-sm font-medium ${active ? 'bg-blue-50 text-blue-700' : 'text-gray-600 hover:bg-gray-50'}`}
                        >
                          <span className="truncate">{group.name}</span>
                          <span className={`shrink-0 rounded-full px-1.5 py-0.5 text-xs font-medium ${active ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-500'}`}>{group.items.length}</span>
                        </button>
                      );
                    })}
                  </>
                ...
                {incomeCategoryGroups.length > 0 && (
                  <>
                    <p className="px-2.5 pb-1 pt-3 text-[11px] font-semibold uppercase tracking-wide text-gray-400">Income</p>
                    {incomeCategoryGroups.map(group => {
                      const active = selectedEntry?.type === 'income' && selectedEntry.name === group.name;
                      return (
                        <button
                          key={`income-${group.name}`}
                          type="button"
                          onClick={() => setSelectedEntry({ type: 'income', name: group.name })}
                          className={`flex items-center justify-between gap-2 rounded-lg px-2.5 py-2 text-left text-sm font-medium ${active ? 'bg-emerald-50 text-emerald-700' : 'text-gray-600 hover:bg-gray-50'}`}
                        >
                          <span className="truncate">{group.name}</span>
                          <span className={`shrink-0 rounded-full px-1.5 py-0.5 text-xs font-medium ${active ? 'bg-emerald-100 text-emerald-700' : 'bg-gray-100 text-gray-500'}`}>{group.items.length}</span>
                        </button>
                      );
                    })}
                  </>
              ...
              <div className="border-t border-gray-100 p-1.5">
                {addingCategory ? (
                  ...
                  <button type="button" onClick={() => setAddingCategory(true)} className="flex w-full items-center gap-2 rounded-lg px-2.5 py-2 text-left text-sm font-medium text-gray-500 hover:bg-gray-50 hover:text-gray-800">
                    <span className="text-base leading-none">+</span> Add expense category
                  </button>
                )}
              </div>
            </aside>
              ...
              {selectedExpenseGroup ? (
                <>
                  <div className="group/cat flex items-center gap-1 border-b border-gray-200 px-4 py-3">
                    {renamingCategory === selectedExpenseGroup.name ? (
                      <form onSubmit={e => { e.preventDefault(); commitRenameCategory(selectedExpenseGroup.name); }} className="flex flex-1 items-center gap-2">
                        <input autoFocus value={renameValue} onChange={e => setRenameValue(e.target.value)} onBlur={() => commitRenameCategory(selectedExpenseGroup.name)} className="min-w-0 flex-1 rounded border border-gray-300 px-2 py-1 text-base font-semibold text-gray-900 outline-none focus:border-blue-500" />
                      </form>
                    ) : (
                      <div className="flex flex-1 items-center gap-2">
                        <h3 className="text-base font-semibold text-gray-900">{selectedExpenseGroup.name}</h3>
                        <span className="rounded-full bg-gray-100 px-1.5 py-0.5 text-xs font-medium text-gray-500">{selectedExpenseGroup.items.length}</span>
                        <span className="ml-auto text-sm font-semibold text-gray-700">{showValues ? formatCurrency(selectedExpenseGroup.subtotal) : '••••'}</span>
                      </div>
                    )}
                    {selectedExpenseGroup.canManage && renamingCategory !== selectedExpenseGroup.name && (
                      <div className="flex items-center gap-0.5 text-gray-400">
                        <button type="button" onClick={() => startRenameCategory(selectedExpenseGroup.name)} title="Rename category" className="rounded p-1.5 hover:bg-gray-100 hover:text-gray-600"><PencilIcon /></button>
                        <button type="button" onClick={() => handleDeleteCategory(selectedExpenseGroup.name)} title="Delete category" className="rounded p-1.5 hover:bg-red-100 hover:text-red-600"><TrashIcon /></button>
                      </div>
                    )}
                  </div>
                  ...
                  {selectedExpenseGroup.items.length === 0 && <p className="px-4 py-8 text-center text-sm text-gray-400">No expenses in this category</p>}
                  {selectedExpenseGroup.items.map(expenseRow)}
                  {categoryQuickAdd('expense', selectedExpenseGroup.name)}
                </>
              ) : selectedIncomeGroup ? (
                <>
                  <div className="flex items-center gap-1 border-b border-gray-200 px-4 py-3">
                    <div className="flex flex-1 items-center gap-2">
                      <h3 className="text-base font-semibold text-gray-900">{selectedIncomeGroup.name}</h3>
                      <span className="rounded-full bg-emerald-50 px-1.5 py-0.5 text-xs font-medium text-emerald-700">{selectedIncomeGroup.items.length}</span>
                      <span className="ml-auto text-sm font-semibold text-emerald-700">{showValues ? formatCurrency(selectedIncomeGroup.subtotal) : '••••'}</span>
                    </div>
                  </div>

                  <div className="hidden items-center gap-2 border-b border-gray-200 px-4 text-xs font-medium uppercase tracking-wide text-gray-400 sm:flex">
                    <span className="min-w-0 flex-1 py-2.5">Description</span>
                    <span className="w-32 shrink-0 py-2.5">Client</span>
                    <span className="w-28 shrink-0 py-2.5">Date</span>
                    <span className="w-32 shrink-0 py-2.5">Category</span>
                    <span className="w-24 shrink-0 py-2.5 text-right">Amount</span>
                    <span className="w-16 shrink-0" />
                  ...
                  {selectedIncomeGroup.items.length === 0 && <p className="px-4 py-8 text-center text-sm text-gray-400">No payments in this category</p>}
                  {selectedIncomeGroup.items.map(incomeRow)}
                  {categoryQuickAdd('income', selectedIncomeGroup.name)}
                </>
              ) : (
