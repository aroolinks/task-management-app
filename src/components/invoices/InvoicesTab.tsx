...
// archive and the create form, so both are reachable without leaving the main
// page shell. The standalone /invoices and /invoices/new routes still work too.
  ...
  return (
    <div>
      <div className="flex gap-1 border-b border-slate-200 bg-slate-100 px-4 py-2 sm:px-6">
        ...
        <button type="button" onClick={() => setView('new')} className={tabClass(view === 'new')}>+ New invoice</button>
      </div>
      {view === 'archive' ? <InvoiceArchive /> : <InvoiceForm />}
    </div>
  );
}
