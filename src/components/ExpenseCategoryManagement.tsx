...
export default function ExpenseCategoryManagement() {
  const { expenseCategories, loading, error, addExpenseCategory, renameExpenseCategory, removeExpenseCategory } = useExpenseCategories();
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    if (!trimmed) return;
    setSubmitting(true);
    const success = await addExpenseCategory(trimmed);
    setSubmitting(false);
  ...
  const handleDelete = async (id: string, name: string) => {
    if (window.confirm(`Delete expense category "${name}"? Existing expenses using it will keep the value, but it won't be selectable anymore.`)) {
  ...
  return (
    <div className="mt-6 bg-white border border-gray-200 rounded-lg overflow-hidden">
      <div className="px-6 py-4 border-b border-gray-200">
        ...
        <p className="text-sm text-gray-600 mt-1">Manage the options available in an expense&apos;s &quot;Category&quot; field.</p>
      </div>
      ...
      <div className="p-6 space-y-4">
        ...
        {loading ? (
          <div className="text-sm text-gray-500">Loading expense categories...</div>
        ) : expenseCategories.length === 0 ? (
          <div className="text-sm text-gray-500">No expense categories yet. Add one above.</div>
        ) : (
          <div className="flex flex-wrap gap-2">
            {expenseCategories.map(ec => (
              <div key={ec.id} className="flex items-center gap-1.5 pl-3 pr-1.5 py-1.5 bg-gray-50 border border-gray-200 rounded-lg">
                {editingId === ec.id ? (
                  <input
                    type="text"
                    value={editingName}
                    onChange={(e) => setEditingName(e.target.value)}
                    onBlur={() => commitRename(ec.id)}
                    onKeyDown={(e) => {
                      if (e.key === 'Enter') commitRename(ec.id);
                  ...
                  <button
                    type="button"
                    onClick={() => startEditing(ec.id, ec.name)}
                ...
                <button
                  type="button"
                  onClick={() => handleDelete(ec.id, ec.name)}
